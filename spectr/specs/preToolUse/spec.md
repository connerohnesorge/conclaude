...
The system SHALL prevent Claude from editing specified files using glob patterns in the `uneditableFiles` configuration. The configuration supports both simple string format (backward compatible) and detailed object format with optional agent-aware filtering.
...
The system SHALL support optional agent-specific filtering in `uneditableFiles` rules, allowing different protection policies for the main orchestrator session versus subagents.

#### Scenario: Agent field defaults to all agents
- **WHEN** `preToolUse.uneditableFiles` contains a rule without an `agent` field
- **THEN** the rule SHALL apply to ALL agents (equivalent to `agent: "*"`)
- **AND** both the main session and all subagents SHALL be blocked from modifying matching files

#### Scenario: Explicit wildcard agent pattern
- **WHEN** `preToolUse.uneditableFiles` contains a rule with `agent: "*"`
- **THEN** the rule SHALL explicitly apply to all agents
- **AND** behavior SHALL be identical to omitting the agent field

#### Scenario: Main session only protection
- **WHEN** `preToolUse.uneditableFiles` contains a rule with `agent: "main"`
- **THEN** the rule SHALL ONLY apply to the main/orchestrator session
- **AND** subagents like "coder", "tester", etc. SHALL NOT be blocked by this rule

#### Scenario: Specific subagent protection
- **WHEN** `preToolUse.uneditableFiles` contains a rule with `agent: "coder"`
- **THEN** the rule SHALL ONLY apply to the "coder" subagent
- **AND** the main session and other subagents SHALL NOT be blocked by this rule

#### Scenario: Glob pattern agent matching
- **WHEN** `preToolUse.uneditableFiles` contains a rule with `agent: "code*"`
- **THEN** the rule SHALL apply to any agent whose identifier matches the glob pattern
- **AND** agents like "coder", "coder-v2", "code-review" SHALL be blocked
- **AND** agents like "tester", "main" SHALL NOT be blocked

#### Scenario: Agent filtering happens before file pattern matching
- **WHEN** a file operation is evaluated against uneditableFiles rules
- **THEN** the system SHALL first check if the current agent matches the rule's agent pattern
- **AND** if the agent does not match, the rule SHALL be skipped entirely
- **AND** only rules where the agent matches SHALL have their file patterns evaluated

#### Scenario: Multiple rules with different agent filters
- **WHEN** `preToolUse.uneditableFiles` contains multiple rules with different agent filters
- **AND** a file operation is performed
- **THEN** the system SHALL evaluate ALL rules where the agent pattern matches
- **AND** if any matching rule's file pattern matches, the operation SHALL be blocked

#### Scenario: Backward compatibility with simple string format
- **WHEN** `preToolUse.uneditableFiles` contains a simple string like `".env"`
- **THEN** the system SHALL treat it as equivalent to `{ pattern: ".env", agent: "*" }`
- **AND** the rule SHALL apply to all agents

#### Scenario: Agent-specific protection example
- **WHEN** configuration contains:
  ...
  ```
- **AND** the "coder" subagent attempts to edit `spectr/changes/123/tasks.jsonc`
- **THEN** the operation SHALL be blocked
- **AND** the error message SHALL include the custom message
- **AND** if the main session attempts the same edit, it SHALL be allowed

#### Scenario: Orchestrator-only protection example
- **WHEN** configuration contains:
  ```yaml
  uneditableFiles:
    - pattern: "*.md"
      agent: "code*"
      message: "Markdown files should be modified by orchestrator only"
  ```
- **AND** any agent matching "code*" (e.g., "coder") attempts to edit any `.md` file
- **THEN** the operation SHALL be blocked
- **AND** the main orchestrator session SHALL be allowed to edit `.md` files
- **AND** the "tester" subagent SHALL also be allowed to edit `.md` files

#### Scenario: Agent identifier for main session
- **WHEN** the system evaluates agent patterns
- **THEN** the main/orchestrator session SHALL be identified as "main"
- **AND** rules with `agent: "main"` SHALL only apply to the orchestrator
- **AND** rules with agent patterns not matching "main" SHALL NOT apply to the orchestrator
